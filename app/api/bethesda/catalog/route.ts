...
export async function GET(request: NextRequest) {
  ...
  const requestedPlatforms = (
    incoming.get("hardware_platforms") || "WINDOWS,PLAYSTATION5,XBOXSERIESX"
  )
    .split(",")
  ...
  const params = new URLSearchParams({
    ...
    order,
    hardware_platforms: (requestedPlatforms.length
      ? requestedPlatforms
      : [...allowedPlatforms]
    ).join(","),
    deleted: "false",
  ...
  return NextResponse.json({
    ...
    total:
      (typeof catalog.total === "number" ? catalog.total : 0) +
